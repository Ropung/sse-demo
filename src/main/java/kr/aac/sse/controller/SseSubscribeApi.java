...
public final class SseSubscribeApi {
    ...
    public ResponseEntity<SseEmitter> subscribeForInquiry(/* (기본적으로) 파라미터 불필요 */ HttpServletResponse response) {
        // 리버스 프록시에서 불필요한 버퍼링을 방지
        response.addHeader("X-Accel-Buffering", "no");

        // response 때 클라이언트 정보와 연계되는 것으로 추정
        return ResponseEntity.ok(inquirySubscribeUseCase.subscribe());
    }
}
